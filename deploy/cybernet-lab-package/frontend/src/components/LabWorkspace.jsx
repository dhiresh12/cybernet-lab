...
export default function LabWorkspace({ lab, onExit, onComplete }) {
  ...
  const [tshootPacketFlow, setTshootPacketFlow] = useState([]);
  const [tshootFailure, setTshootFailure] = useState(null);
  const [tshootSolved, setTshootSolved] = useState(false);
  const audioRef = useRef(null);
  const engineRef = useRef(null);
  const tshootEngineRef = useRef(null);
  ...
  useEffect(() => {
    ...
    // Initialize engine with devices
    devices.forEach(d => {
      ...
      Object.entries(deviceState.interfaces).forEach(([name, iface]) => {
        if (iface.ip !== 'unassigned') {
          ...
          if (iface.description) {
            engine.processCommand(d.id, `description ${iface.description}`);
          }
        }
      });
    });

    // Keep deviceStates in sync with engine
    ...
    engine.on('device:removed', syncDeviceStates);

    return () => {
      engine.off('device:stateChanged', syncDeviceStates);
      engine.off('device:created', syncDeviceStates);
      engine.off('device:removed', syncDeviceStates);
      engine.reset();
    };
  }, [lab]);
  ...
  useEffect(() => {
    ...
    const getEdgeStatus = (fromId, toId, deviceStates) => {
  const fromDs = deviceStates[fromId];
  const toDs = deviceStates[toId];
  if (!fromDs || !toDs) return 'down';
  const fromUp = Object.values(fromDs.interfaces).some(i => i.status === 'up');
  const toUp = Object.values(toDs.interfaces).some(i => i.status === 'up');
...
const edges = connections.map((c, idx) => {
  ...
  return {
    ...
    status: getEdgeStatus(fromDev?.id || `dev-${fromName}`, toDev?.id || `dev-${toName}`, deviceStates),
    label: c,
    cableType: 'Ethernet'
  };
}).filter(e => e.from && e.to);
setTopologyEdges(edges);
  }, [lab, devices, connections, ipTable, layout, deviceStates]);
    ...
    const interval = setInterval(() => {
      setPacketProgress(prev => {
        ...
        if (np >= 1) {
          ...
          if (prev.mode === 'trace') {
            setTimeout(() => setPacketProgress(null), 200);
    ...
    try {
      ...
      if (result.passed) {
        ...
        if (currentStepIdx < steps.length - 1) {
          ...
          setVerifying(false);
          // Pass xp via callback; LabWorkspace will call onComplete
        }
      } else {
        setVerifying(false);
      }
...
const handleSendCommand = useCallback((cmd) => {
    if (!cmd.trim()) return;
    if (!activeTerminalDevice || !engineRef.current) return;
    ...
    // Handle show commands specially to use engine renderers
    ...
    if (lower.startsWith('show ')) {
      ...
      if (lower === 'show running-config' || lower === 'show run') {
        output = engine.showRunningConfig(activeTerminalDevice);
      } else if (lower === 'show ip interface brief' || lower === 'show ip int br') {
        output = engine.showIpInterfaceBrief(activeTerminalDevice);
      } else if (lower === 'show ip route') {
        output = engine.showIpRoute(activeTerminalDevice);
      } else if (lower === 'show interfaces') {
        output = engine.showInterfaces(activeTerminalDevice);
      } else if (lower === 'show arp') {
        output = engine.showArp(activeTerminalDevice);
      } else if (lower === 'show vlan brief' || lower === 'show vlan') {
        output = engine.showVlanBrief(activeTerminalDevice);
      } else if (lower === 'show mac address-table') {
        // Not implemented in engine yet - fallback
        output = ['% Command not implemented'];
      } else if (lower === 'show access-lists' || lower === 'show access-list') {
        output = engine.showAccessLists(activeTerminalDevice);
      } else if (lower === 'show ip nat translations') {
        output = engine.showIpNatTranslations(activeTerminalDevice);
      } else if (lower === 'show ip dhcp binding') {
        output = engine.showIpDhcpBinding(activeTerminalDevice);
      } else if (lower === 'show ip ssh') {
        output = engine.showIpSsh(activeTerminalDevice);
      } else if (lower === 'show port-security address') {
        output = engine.showPortSecurityAddress(activeTerminalDevice);
      } else if (lower === 'show spanning-tree' || lower === 'show spanning') {
        output = engine.showSpanningTree(activeTerminalDevice);
      } else if (lower === 'show etherchannel summary') {
        output = engine.showEtherchannelSummary(activeTerminalDevice);
      } else if (lower === 'show ip eigrp neighbors') {
        output = engine.showIpEigrpNeighbors(activeTerminalDevice);
      } else if (lower === 'show ip ospf neighbor') {
        output = engine.showIpOspfNeighbor(activeTerminalDevice);
      } else if (lower === 'show ip bgp summary') {
        output = engine.showIpBgpSummary(activeTerminalDevice);
      } else if (lower === 'show ip protocols') {
        output = engine.showIpProtocols(activeTerminalDevice);
      } else if (lower === 'show ntp status') {
        output = engine.showNtpStatus(activeTerminalDevice);
      } else {
        // Unknown show command
        output = engine.processCommand(activeTerminalDevice, cmd).output;
      }

      output.forEach(line => newLines.push({ text: line, type: 'output' }));
      setTerminalOutput(newLines);
      setTerminalInput('');
      return;
    }

    // Process config/exec commands through engine
    const result = engine.processCommand(activeTerminalDevice, cmd);
    const newLines = [...terminalOutput];
    newLines.push({ text: `${prompt}${cmd}`, type: 'command' });
    result.output.forEach(line => newLines.push({ text: line, type: 'output' }));
    setTerminalOutput(newLines);
    setTerminalInput('');
  ...
  const handlePing = useCallback((targetDeviceId) => {
    ...
    const srcId = selectedDevice || topologyNodes[0]?.id;
    const path = findPath(srcId, targetDeviceId, topologyEdges, topologyNodes);
    setPacketProgress({ from: srcId, to: targetDeviceId, path, progress: 0, edgeIdx: 0, mode: 'ping' });
    setPacketDropped(null);

    if (engineRef.current) {
      const targetIp = ipTable.find(i => i.device === targetNode.name)?.ip || targetNode.ip;
      const result = engineRef.current.simulatePing(srcId, targetIp || '8.8.8.8');
      setTerminalOutput(prev => [...prev, ...result.output.map(o => ({ text: o, type: 'output' }))]);
    }
  }, [selectedDevice, topologyNodes, topologyEdges, ipTable]);
  ...
  const handleTrace = useCallback((targetDeviceId) => {
    ...
    const srcId = selectedDevice || topologyNodes[0]?.id;
    const path = findPath(srcId, targetDeviceId, topologyEdges, topologyNodes);
    setPacketProgress({ from: srcId, to: targetDeviceId, path, progress: 0, edgeIdx: 0, mode: 'trace' });
    setPacketDropped(null);

    if (engineRef.current) {
      const targetIp = ipTable.find(i => i.device === targetNode.name)?.ip || targetNode.ip;
      const result = engineRef.current.simulateTraceroute(srcId, targetIp || '8.8.8.8');
      setTerminalOutput(prev => [...prev, ...result.output.map(o => ({ text: o, type: 'output' }))]);
    }
  }, [selectedDevice, topologyNodes, topologyEdges, ipTable]);

  const findPath = (fromId, toId, edges, nodes) => {
    const graph = {};
  ...
  const formatDeviceState = (deviceId) => {
    ...
    const ds = engineRef.current.getDevice(deviceId);
    ...
    lines.push('');
    lines.push(`SSH: ${ds.ssh.enabled ? 'Enabled' : 'Disabled'}`);
    lines.push(`Telnet: ${ds.ssh.enabled ? 'Enabled' : 'Disabled'} // Telnet not modeled`);
    lines.push(`ACL Applied: ${Object.keys(ds.acl.applied).length > 0 ? 'Yes' : 'No'}`);
    return lines.join('\n');
  };
  ...
  const getSecurityInfo = (deviceId) => {
    ...
    if (!ds) return [];
    const items = [];
    items.push({ label: 'SSH', value: ds.ssh.enabled ? 'Enabled' : 'Disabled', color: ds.ssh.enabled ? 'var(--green)' : 'var(--red)' });
    items.push({ label: 'Telnet', value: 'N/A', color: 'var(--muted)' });
    items.push({ label: 'Enable Secret', value: 'N/A', color: 'var(--muted)' });
    items.push({ label: 'Port Security', value: Object.keys(ds.portSecurity).length > 0 ? 'Applied' : 'N/A', color: Object.keys(ds.portSecurity).length > 0 ? 'var(--green)' : 'var(--muted)' });
    return items;
  };

  useEffect(() => {
    if (!tshootEngineRef.current) {
      tshootEngineRef.current = new TroubleshootingEngine();
    }
  }, []);

  const handleLoadTshootScenario = useCallback((scenarioId) => {
    if (!tshootEngineRef.current || !scenarioId) return;
    const scenario = tshootEngineRef.current.loadScenario(scenarioId);
    if (scenario && engineRef.current) {
      setTshootScenario(scenario);
      setTshootPacketFlow([]);
      setTshootFailure(null);
      setTshootSolved(false);
      tshootEngineRef.current.injectFault(engineRef.current);
    }
  }, []);

  const handleRunPacketTrace = useCallback(() => {
    if (!tshootEngineRef.current || !tshootScenario || !engineRef.current) return;
    const result = tshootEngineRef.current.checkFix(engineRef.current);
    setTshootPacketFlow(result.flow || []);
    if (!result.solved) {
      const fp = tshootEngineRef.current.getFailurePoint();
      setTshootFailure(fp);
      setTshootSolved(false);
    } else {
      setTshootFailure(null);
  ...
  const handleTshootHint = useCallback(() => {
    if (!tshootEngineRef.current) return;
    const hint = tshootEngineRef.current.getHint();
    if (hint) {
      setTerminalOutput(prev => [...prev, { text: `[HINT ${hint.level}]: ${hint.text}`, type: 'hint' }]);
    }
  }, []);
        ...
        {troubleshootMode && (
          <div style={{ marginTop: 12, padding: 8, background: 'rgba(255,51,85,0.05)', border: '1px solid rgba(255,51,85,0.3)', borderRadius: 4 }}>
            ...
            <select
              ...
              style={{ background: 'rgba(0,0,0,0.5)', color: 'var(--red)', border: '1px solid rgba(255,51,85,0.4)', borderRadius: 4, padding: '4px', fontSize: '0.7em', width: '100%' }}
            >
              <option value="">Select scenario...</option>
              {tshootEngineRef.current?.getScenarioList().map(s => (
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      edges.forEach(edge => {
        ...
        if (packetProgress && packetProgress.path && packetProgress.path.length > 1) {
          const totalEdges = packetProgress.path.length - 1;
          const edgeProgress = packetProgress.progress * totalEdges;
          const currentEdgeIdx = Math.floor(edgeProgress);
          const progressInEdge = edgeProgress - currentEdgeIdx;
          
          if (currentEdgeIdx < totalEdges) {
            const fromId = packetProgress.path[currentEdgeIdx];
            const toId = packetProgress.path[currentEdgeIdx + 1];
            const fromNode = nodes.find(n => n.id === fromId);
            const toNode = nodes.find(n => n.id === toId);
            
            if (fromNode && toNode) {
              const x1 = fromNode.x * w + pan.x;
              const y1 = fromNode.y * h + pan.y;
              const x2 = toNode.x * w + pan.x;
              const y2 = toNode.y * h + pan.y;
              
              const px = x1 + (x2 - x1) * progressInEdge;
              const py = y1 + (y2 - y1) * progressInEdge;
              
              ctx.fillStyle = packetProgress.mode === 'trace' ? '#ffaa00' : '#00ff88';
              ctx.shadowBlur = 15;
              ctx.shadowColor = packetProgress.mode === 'trace' ? '#ffaa00' : '#00ff88';
              ctx.beginPath();
        ...
        } else if (packetProgress && packetProgress.from === edge.from && packetProgress.to === edge.to) {
