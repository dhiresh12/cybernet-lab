...
export default function SecurityScan({ data }) {
  if (!data) {
    return <div style={{ color: 'var(--text-dim)', padding: '24px', textAlign: 'center' }}>No security data available. Run QA check to populate.</div>;
  }
  ...
  return (
    <div className="panel">
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px', marginBottom: '16px' }}>
        ...
        <MetricCard title="Secrets" value={data.secretsFound || 0} status={data.secretsFound > 0 ? 'failed' : 'passed'} />
      </div>
      <DetailList title="Security Details" items={data.details?.secrets} />
    </div>
  );
}
