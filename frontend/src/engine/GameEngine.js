...
const gameEngine = {
  getProtocolMatchQuestions(count = 10) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const wrongs = shuffleArray(pool.filter(p => p.protocol !== item.protocol)).slice(0, 3).map(p => p.match);
      const opts = shuffleArray([item.match, ...wrongs]);
      questions.push({
        id: `pm-${i}`,
  ...
  getSubnetMasterQuestions(count = 10) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      questions.push({
        ...
        correct: maskStr,
        options: shuffleArray([maskStr, wmStr, '255.255.255.0', '255.255.0.0'])
      });
    }
    return questions;
  },
  ...
  getBugFindQuestions(count = 8) {
    ...
    return shuffleArray(templates).slice(0, count).map((t, i) => ({
      id: `bug-${i}`,
      labTitle: 'Config Review',
      config: t.config,
      correct: t.correct,
      options: shuffleArray(['Wildcard mask', 'subnet mask', 'version', 'mask', 'secret', 'administrative distance', 'no shutdown', 'auto-summary'])
    }));
  },
  ...
  getTroubleshootQuestions(count = 8) {
    ...
    return shuffleArray(cases).slice(0, count).map((c, i) => ({ ...c, id: `ts-${i}` }));
  },
  ...
  getTopologyQuestions(count = 8) {
    ...
    return shuffleArray(scenarios).slice(0, count).map((s, i) => ({ ...s, id: `topo-${i}` }));
  }
};
