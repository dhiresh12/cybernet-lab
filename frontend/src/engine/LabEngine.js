...
export default class LabEngine {
  constructor(audio) {
    ...
    this._activeLabDefinition = null;
    this.state = {
  ...
  async startLab(labId, labDefinition) {
    ...
    this._activeLabDefinition = labDefinition || { id: labId, steps: [] };
    this.state.active = true;
  ...
  _currentLabStep(stepId) {
    const steps = Array.isArray(this._activeLabDefinition?.steps)
      ? this._activeLabDefinition.steps
      : [];
    return steps.find(step => String(step.stepId || step.id) === String(stepId)) || null;
  }
  ...
  attachSimulationEngine(simulationEngine) {
    ...
    // Sync device states to backend when runtime changes
    this._runtimeSyncUnsub = this.on('runtime:changed', (runtime) => {
      if (!this._connected || this._localMode) return;
      if (!runtime || !runtime.devices) return;
      Object.entries(runtime.devices).forEach(([deviceId, device]) => {
        this._send({ type: 'device:state', deviceId, state: device });
      });
    });
  ...
  detachSimulationEngine() {
    ...
    if (this._runtimeSyncUnsub) {
      this._runtimeSyncUnsub();
      this._runtimeSyncUnsub = null;
    }
    return this;
  }
