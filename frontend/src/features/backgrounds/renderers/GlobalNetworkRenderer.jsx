...
export const GlobalNetworkRenderer = ({ opacity = 0.8, zIndex = -2 }) => {
  ...
  useEffect(() => {
    ...
    const project = (lat, lon, cx, cy, radius) => {
      const x = cx + radius * Math.cos(lat * Math.PI / 180) * Math.cos(lon * Math.PI / 180);
    ...
    const draw = () => {
      ...
      ctx.lineWidth = 1.5;
      hubs.forEach(h => {
        const p1 = project(h.lat, h.lon, cx, cy, radius);
        h.connections.forEach(connName => {
          const conn = hubMap[connName];
          if (conn) {
            const p2 = project(conn.lat, conn.lon, cx, cy, radius);
            ctx.beginPath();
      ...
      // Hubs
      hubs.forEach(h => {
        const p = project(h.lat, h.lon, cx, cy, radius);
        const pulse = Math.sin(Date.now() * 0.003 + h.lon) * 0.5 + 0.5;
